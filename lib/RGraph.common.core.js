...
//    };
    ...
    RGraph.Background.draw = function ()
    {
        ...
        var prop         = args.object.properties,
            ...
            // X axis title (not for the Bar chart now - that's done
            // in the newer drawXAxis() function)
            if (args.object.type !== 'bar' && typeof prop.xaxisTitle === 'string' && prop.xaxisTitle.length) {
...
//        //}
    ...
    RGraph.measureText = function ()
    {
        ...
        div.innerHTML        = args.text.replace(/\r?\n/g, '<br />');
        ...
        function domtext ()
        {
            ...
            if (!RGraph.text.domNodeCache[obj.id] || !RGraph.text.domNodeCache[obj.id][cacheKey]) {
                ...
                var span = document.createElement('span');
                    ...
                    span.style.fontSize   = (args.size || defaults.size) + 'pt';
    ...
    RGraph.isNull = function ()
    {
        ...
        if (args.arg == null || typeof args.arg === 'object' && !args.arg) {
            return true;
        }
    ...
    //
    // myBar.path('lw 10 b r % % % % s black f red', 5, 5, 50, 50)
    //
    ...
    RGraph.drawXAxis = function (obj)
    {
        ...
        if (prop.xaxis) {
            ...
            obj.path(
                'lw % b m % % l % % s %',
                prop.xaxisLinewidth,
                prop.marginLeft - (isSketch ? 5 : 0), obj.getYCoord(prop.yaxisScaleMin > 0 ? prop.yaxisScaleMin : 0) - (isSketch ? 2 : 0),
                obj.canvas.width - prop.marginRight + (isSketch ? 7 : 0), obj.getYCoord(prop.yaxisScaleMin > 0 ? prop.yaxisScaleMin : 0) + (isSketch ? 2 : 0),
                prop.xaxisColor
            );
            ...
            if (!isSketch) {
                if (prop.xaxisTickmarks) {
                    ...
                    // TODO get number of tickmarks from the number of data points
                    } else if (obj.type === 'bar') {
                        var xaxisTickmarksCount = obj.data.length;
    
                    //  Default to 10 tickmarks
                    } else {
                ...
                if (prop.xaxisPosition === 'center' && prop.yaxisScaleMin > 0) {
                    ...
                    var tickmarksYStart = obj.getYCoord(0);
                    var tickmarksYEnd   = obj.getYCoord(0) + tickmarksLength;
                }
                ...
                for (var i=0; i<=xaxisTickmarksCount; ++i) {
                    ...
                    if (RGraph.isNull(prop.xaxisTickmarksLastLeft)) {
                        if ( i === 0 && prop.yaxis && prop.yaxisPosition === 'left' ) {
                            continue;
                        }
        ...
        if (prop.xaxisLabels && prop.xaxisLabels.length && prop.xaxisLabelsPosition === 'section') {
            ...
            for (var i=0; i<prop.xaxisLabels.length; ++i) {
                ...
                RGraph.text({
                  ...
                  object: obj,
                    ...
                    y:      prop.xaxisPosition === 'top' ? obj.marginTop + prop.xaxisLabelsOffsety - 5 : (obj.canvas.height - prop.marginBottom) + prop.xaxisLabelsOffsety + 5,
        ...
        if (prop.xaxisTitle) {
            ...
            var y = prop.xaxisPosition === 'top' ? prop.marginTop  - 7 + prop.xaxisTitleOffsety : obj.canvas.height - prop.marginBottom + 7 + prop.xaxisTitleOffsety;
            
            
            // Get the size of the X axis labels
            if (prop.xaxisLabels && prop.xaxisLabels.length) {
                var textConf = RGraph.getTextConf({
                    object: obj,
                    prefix: 'xaxisLabels'
                });
            ...
            // Get the configuration of the X axis title
            var textConf = RGraph.getTextConf({
                object: obj,
                prefix: 'xaxisTitle'
            });
